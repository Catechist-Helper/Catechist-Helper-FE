...
const ListAllTrainCatechist: React.FC = () => {
  ...
  const handleRemoveCatechistFromTraining = async (catechistId: string) => {
    ...
    setCatechists(prevCatechists => prevCatechists.filter(cat => cat.id !== catechistId));
  ...
  const updateStatus = async (catechistId: string, newStatus: number) => {
    try {
      ...
      if (trainingStatus === 0) { // Chưa bắt đầu
        if (newStatus === 2) {
          alert("Không thể đặt trạng thái thành 'Hoàn thành' khi khóa đào tạo chưa bắt đầu.");
          return;
        }
      ...
      const response: { status: number } = await catInTrainApi.updateCatInTrain(id!, payload);
        ...
        <div className="flex flex-col md:flex-row justify-between gap-8 px-4">
          <div className="flex-1 space-y-4 pl-10">
            <p>
              ...
              <span className="text-gray-700 ml-2">Cấp {trainingData?.previousLevel?.hierarchyLevel}</span>
            </p>
            <p>
              <span className="font-semibold text-[#6B4423] text-lg pl-20">Cấp bậc sau khi hoàn thành:</span>
              <span className="text-gray-700 ml-2">Cấp {trainingData?.nextLevel?.hierarchyLevel}</span>
            </p>
            <p>
          ...
          <div className="flex-1 space-y-4">
            <p>
              ...
              <span className="text-gray-700 ml-2">{trainingData?.description}</span>
            </p>
            <p>
            ...
            <table className="w-full text-sm text-left">
              <thead className="text-sm text-white uppercase bg-[#422A14] h-12">
                <tr className="text-center">
                  <th scope="col" className="px-6 py-3 w-1/2">Tên giáo lý viên</th>
                  <th scope="col" className="px-6 py-3 w-1/2">Trạng thái</th>
                </tr>
              </thead>
                  ...
                  <tr key={catechist.id} className="bg-white hover:bg-gray-50">
                    ...
                    <td className="px-6 py-3 text-center">
                      <select
                        ...
                        onChange={(e) => updateStatus(catechist.id, parseInt(e.target.value, 10))}
                      >
                        {Object.entries(catechistInTrainingStatusLabel).map(([key, label]) => {
                          ...
                          const isDisabled =
                            ...
                            (trainingStatus === 1 && (keyNum === 0 || keyNum === 2 || (catechist.status === 3 && keyNum === 1))) ||
                            ...
                            (trainingStatus === 2 && (
                              ...
                              (catechist.status === 2 && (keyNum === 1 || keyNum === 3)) ||
                              // Nếu catechist đang ở trạng thái Không đạt (3)
                              (catechist.status === 3 && (keyNum === 1 || keyNum === 2))
                            ));
