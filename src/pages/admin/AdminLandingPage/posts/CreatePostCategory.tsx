...
const CreatePostCategory: React.FC = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const response = await postCategoryApi.create(values.name, values.description);
            ...
            <form onSubmit={formik.handleSubmit} className="max-w-sm mx-auto mt-5">
                ...
                <div className="flex items-start mb-5">
                    <button
                        ...
                        disabled={isSubmitting} onClick={handleGoBack}
                    >
                        {isSubmitting ? "Đang tạo" : "Tạo danh mục bài viết"}
                    </button>
                    <Link to="/admin" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center ml-5">
                        Back
                    </Link>
                </div>
            </form>
